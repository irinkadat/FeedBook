...
const App = () => {
  ...
  const [favorites, setFavorites] = useState(() => {
    ...
    try {
      // Parse the storedFavorites only if it's a truthy string and an array
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (authUser) => {
      setUser(authUser);
      setLoading(false); // Update loading state once authentication state is determined
    });
  ...
  const updateFavorites = (newFavorites) => {
    ...
    const favoritesArray = Array.isArray(newFavorites) ? newFavorites : [newFavorites];

    // Log type and content of newFavorites
  ...
  const handleAddToFavorites = (newFavorite) => {
    // Append the newFavorite to the existing favorites array
  ...
  const handleRemoveFromFavorites = (recipeToRemove) => {
    // Filter out the recipe to be removed based on title
    const updatedFavorites = favorites.filter((recipe) => recipe.title !== recipeToRemove.title);
  ...
  return (
    ...
    <Router>
      ...
      <Routes>
        <Route path="/" element={<RecipeListPage onAddToFavorites={handleAddToFavorites} />} />
